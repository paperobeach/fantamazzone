...
$squadre = query_all("SELECT id FROM SQUADRE WHERE stagione = $stagione");
...
foreach ($squadre as $sq) {
    ...
    $ultime = query_all("SELECT segno FROM RISULTATI
                         WHERE stagione = $stagione AND id_squadra = $id
                         ORDER BY giornata DESC LIMIT 1");
    $ultime_osp = query_all("SELECT segno FROM RISULTATI
    ...
    // Upsert GENERALE
    $exists = query_one("SELECT COUNT(*) AS n FROM GENERALE
                         WHERE stagione = $stagione AND id_squadra = $id");
    if ((int)$exists["n"] > 0) {
        mysqli_query($conn, "UPDATE GENERALE SET
                punti     = $punti,
                partiteg  = $pg,
                vinte     = $vinte,
                nulle     = $nulle,
        ...
        $sq_info = query_one("SELECT nome, logo FROM SQUADRE
        ...
        mysqli_query($conn, "INSERT INTO GENERALE
