...
if ($method === "POST") {
    ...
    mysqli_query($conn, "INSERT INTO NEW_PENALITA (stagione, id_squadra, punti, motivo)
    ...
    $id = param_int("id");
    mysqli_query($conn, "DELETE FROM NEW_PENALITA WHERE stagione = $stagione AND id = $id");
    ...
    $sql = "SELECT p.id, p.id_squadra, s.nome AS squadra, s.logo, p.punti, p.motivo
            FROM NEW_PENALITA p
            JOIN NEW_SQUADRE s ON s.id = p.id_squadra AND s.stagione = p.stagione
